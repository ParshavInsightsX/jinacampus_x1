export const STAFFBOARD_LITE_AUDIT_EVENTS = {
  STAFF_CREATED: "staffboard.staff.created",
  STAFF_UPDATED: "staffboard.staff.updated",
  STAFF_DEACTIVATED: "staffboard.staff.deactivated",
  STAFF_EMPLOYMENT_STATUS_CHANGED: "staffboard.staff.employment_status_changed",
  STAFF_PROFILE_PHOTO_UPLOADED: "staffboard.staff.photo_uploaded",
  STAFF_PROFILE_PHOTO_REMOVED: "staffboard.staff.photo_removed",
  STAFF_LOGIN_ACCESS_CREATED: "staffboard.staff.login_access_created",
  STAFF_LOGIN_ACCESS_DISABLED: "staffboard.staff.login_access_disabled",
  STAFF_LOGIN_ACCESS_REACTIVATED: "staffboard.staff.login_access_reactivated",
  STAFF_ATTENDANCE_QR_GENERATED: "staffboard.attendance.qr.generated",
  STAFF_ATTENDANCE_QR_REGENERATED: "staffboard.attendance.qr.regenerated",
  STAFF_ATTENDANCE_QR_DEACTIVATED: "staffboard.attendance.qr.deactivated",
  STAFF_ATTENDANCE_QR_EXPIRED: "staffboard.attendance.qr.expired",
  STAFF_ATTENDANCE_QR_USED: "staffboard.attendance.qr.used",
  STAFF_ATTENDANCE_CHECK_IN: "staffboard.attendance.check_in",
  STAFF_ATTENDANCE_CHECK_OUT: "staffboard.attendance.check_out",
  STAFF_ATTENDANCE_CORRECTED: "staffboard.attendance.corrected",
  STAFF_ATTENDANCE_CREDENTIAL_ISSUED: "staffboard.attendance.credential.issued",
  STAFF_ATTENDANCE_CREDENTIAL_REISSUED: "staffboard.attendance.credential.reissued",
  STAFF_ATTENDANCE_CREDENTIAL_REVOKED: "staffboard.attendance.credential.revoked",
  STAFF_ATTENDANCE_CREDENTIAL_VIEWED: "staffboard.attendance.credential.viewed",
  STAFF_ATTENDANCE_CREDENTIAL_PRINTED: "staffboard.attendance.credential.printed",
  STAFF_ATTENDANCE_SCAN_SESSION_STARTED: "staffboard.attendance.scan_session.started",
  STAFF_ATTENDANCE_SCAN_SESSION_CLOSED: "staffboard.attendance.scan_session.closed",
  STAFF_ATTENDANCE_EVENT_RECORDED: "staffboard.attendance.event.recorded",
  STAFF_ATTENDANCE_ADJUSTMENT_REQUESTED: "staffboard.attendance.adjustment.requested",
  STAFF_ATTENDANCE_ADJUSTMENT_APPROVED: "staffboard.attendance.adjustment.approved",
  STAFF_ATTENDANCE_ADJUSTMENT_REJECTED: "staffboard.attendance.adjustment.rejected",
  STAFF_LEAVE_SUBMITTED: "staffboard.leave.submitted",
  STAFF_LEAVE_MODIFIED: "staffboard.leave.modified",
  STAFF_LEAVE_CLARIFICATION_REQUESTED: "staffboard.leave.clarification_requested",
  STAFF_LEAVE_CLARIFICATION_PROVIDED: "staffboard.leave.clarification_provided",
  STAFF_LEAVE_APPROVED: "staffboard.leave.approved",
  STAFF_LEAVE_REJECTED: "staffboard.leave.rejected",
  STAFF_LEAVE_CANCELLED: "staffboard.leave.cancelled",
  STAFF_LEAVE_WITHDRAWN: "staffboard.leave.withdrawn",
  STAFF_LEAVE_SETTING_UPDATED: "staffboard.leave.setting_updated",
  STAFF_LEAVE_TYPE_UPSERTED: "staffboard.leave.type_upserted",
  STAFF_LEAVE_APPROVER_UPDATED: "staffboard.leave.approver_updated",
  STAFF_LEAVE_BALANCE_ADJUSTED: "staffboard.leave.balance_adjusted",
  STAFF_LEAVE_DOCUMENT_UPLOADED: "staffboard.leave.document_uploaded",
  STAFF_LEAVE_DOCUMENT_DELETED: "staffboard.leave.document_deleted"
} as const;
