...
export const updateAttendanceSettingsSchema = z.object({
  ...
  staffAttendanceCaptureMode: z.enum(["SUPERVISED_QR", "MANUAL_ONLY"]).default("SUPERVISED_QR"),
  staffSelfScanEnabled: z.literal(false).default(false),
  staffManualAttendanceEnabled: z.boolean().default(true),
  staffCorrectionApprovalRequired: z.literal(true).default(true),
  staffScanSessionValidityMinutes: z.number().int().min(5).max(240).default(60),
  staffCredentialValidityDays: z.number().int().min(1).max(3650).default(365),
