...
export function GradebookSubnav({ items }: { items: readonly { title: string; href: string }[] }) {
  const pathname = usePathname();
  return (
    <nav aria-label="GradeBook sections" className="overflow-x-auto border-b border-slate-200/80 pb-3">
      <div className="flex min-w-max gap-2">
        {items.map((item) => {
          const active = pathname === item.href || (item.href !== "/gradebook" && pathname.startsWith(`${item.href}/`));
          return (
            <Link
              key={item.href}
              href={item.href}
